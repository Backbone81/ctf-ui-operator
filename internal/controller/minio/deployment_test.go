...
var _ = Describe("DeploymentReconciler", func() {
	...
	BeforeEach(func() {
		reconciler = minio.NewReconciler(k8sClient, minio.WithDeploymentReconciler())
	})
	...
	It("should use the resources provided by the minio resource", func(ctx SpecContext) {
		By("prepare test with all preconditions")

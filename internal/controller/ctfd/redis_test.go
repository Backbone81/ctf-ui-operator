...
var _ = Describe("RedisReconciler", func() {
	...
	BeforeEach(func() {
		reconciler = ctfd.NewReconciler(k8sClient, ctfd.WithRedisReconciler())
	})
	...
	It("should successfully create the Redis", func(ctx SpecContext) {
		...
		Expect(k8sClient.Get(ctx, client.ObjectKey{
			...
			Name:      ctfd.RedisName(&instance),
		}, &redis)).To(Succeed())
		Expect(redis.Spec.Resources).To(BeZero())
	})
	...
	It("should use the resources provided by the ctfd resource", func(ctx SpecContext) {
		...
		Expect(redis.Spec.Resources).To(HaveValue(Equal(resources)))
	})
})

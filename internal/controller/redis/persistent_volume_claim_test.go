...
var _ = Describe("PersistentVolumeClaimReconciler", func() {
	...
	BeforeEach(func() {
		reconciler = redis.NewReconciler(k8sClient, redis.WithPersistentVolumeClaimReconciler())
	})
	...
	It("should use the persistent volume claim provided by the redis resource", func(ctx SpecContext) {
		By("prepare test with all preconditions")
		instance := v1alpha1.Redis{
			ObjectMeta: metav1.ObjectMeta{
				GenerateName: "test-",

...
func (r *PersistentVolumeClaimReconciler) Reconcile(ctx context.Context, mariadb *v1alpha1.MariaDB) (ctrl.Result, error) {
	currentSpec, err := r.getPersistentVolumeClaim(ctx, mariadb)
	if err != nil {
		return ctrl.Result{}, err
	}

	desiredSpec, err := r.getDesiredPersistentVolumeClaimSpec(mariadb)
	if err != nil {
		return ctrl.Result{}, err
	}
...
func (r *PersistentVolumeClaimReconciler) getPersistentVolumeClaim(ctx context.Context, mariadb *v1alpha1.MariaDB) (*corev1.PersistentVolumeClaim, error) {
	var persistentVolumeClaim corev1.PersistentVolumeClaim
	if err := r.GetClient().Get(ctx, client.ObjectKeyFromObject(mariadb), &persistentVolumeClaim); err != nil {
		return nil, client.IgnoreNotFound(err)
	}
	return &persistentVolumeClaim, nil
}

func (r *PersistentVolumeClaimReconciler) getDesiredPersistentVolumeClaimSpec(mariadb *v1alpha1.MariaDB) (*corev1.PersistentVolumeClaim, error) {
	...
	if mariadb.Spec.PersistentVolumeClaim != nil {
		result.Spec = *mariadb.Spec.PersistentVolumeClaim
	}

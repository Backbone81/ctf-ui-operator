...
var _ = Describe("PersistentVolumeClaimReconciler", func() {
	...
	BeforeEach(func() {
		reconciler = mariadb.NewReconciler(k8sClient, mariadb.WithPersistentVolumeClaimReconciler())
	})
	...
	It("should use the persistent volume claim provided by the mariadb resource", func(ctx SpecContext) {
		By("prepare test with all preconditions")
		instance := v1alpha1.MariaDB{
			ObjectMeta: metav1.ObjectMeta{
				GenerateName: "test-",

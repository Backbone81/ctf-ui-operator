...
var _ = Describe("DeploymentReconciler", func() {
	...
	BeforeEach(func() {
		reconciler = mariadb.NewReconciler(k8sClient, mariadb.WithDeploymentReconciler())
	})
	...
	It("should use the resources provided by the mariadb resource", func(ctx SpecContext) {
		By("prepare test with all preconditions")
